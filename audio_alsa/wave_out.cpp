...
//         return true;
      ...
      void wave_out::out_open_ex(thread * pthreadCallback, unsigned int uiSamplesPerSec, unsigned int uiChannelCount, unsigned int uiBitsPerSample, ::wave::enum_purpose epurpose)
      {
...
//         m_pprebuffer->SetMinL1BufferCount(out_get_buffer()->GetBufferCount());
         ...
         if((err = snd_pcm_sw_params_set_tstamp_type(m_ppcm, m_pswparams,SND_PCM_TSTAMP_TYPE_GETTIMEOFDAY)) <0)
         {
            ...
            information("unable to set time stamp type: %s\n", snd_strerror(err));
