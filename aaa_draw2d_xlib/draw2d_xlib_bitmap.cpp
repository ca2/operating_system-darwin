   ...
   bool bitmap::CreateBitmap(::draw2d::graphics * pgraphics, int cx, int cy, unsigned int nPlanes, unsigned int nBitcount, const void * pdata, int iStrideParam)
   {
      ...
      if(iStrideParam != iStride)
      {
         ...
         for(int i = 0; i < cy; i++)
         {
            ...
            ::memcpy_dup(&((unsigned char *) m_mem.get_data())[iStride * i], &((unsigned char *) pdata)[iStrideParam * i], iW);
      ...
      if(*ppdata != nullptr)
      {
         ...
         if(cx * 4 != iStride)
         {
            ...
            for(int i = 0; i < cy; i++)
            {
               ...
               ::memcpy_dup(&((unsigned char *) m_mem.get_data())[iStride * i], &((unsigned char *) *ppdata)[iW * i], iW);
